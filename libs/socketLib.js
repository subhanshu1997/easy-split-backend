    ...
    myIo.sockets.on('connection',(socket) => {
        ...
        socket.on('create-expense',(data)=>{
            for(let item of data.members){
                if(item!=data.userId){
            myIo.sockets.emit(item,"You have been added to an expense with title")}
                }
        })
        ...
        socket.on('delete-expense',(data)=>{
            for(let item of data.members){
                if(item!=data.userId){
            myIo.sockets.emit(item,"Expense with title"+data.expenseName+" has been deleted")}
                }
        })
        ...
        socket.on('update-expense',(data)=>{
            for(let item of data.members){
                if(item!=data.userId){
            myIo.sockets.emit(item,data.message)}
                }
        })
